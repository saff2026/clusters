...
function groupChart(el){
  ...
  const grpDone=arr.filter(a=>a.count>=TARGET).length;
  const gh=document.getElementById('grph'); if(gh)gh.innerHTML+=' <span style="color:#7ee0a0;font-size:12px;font-weight:700">('+grpDone+'/'+arr.length+' مكتملة)</span>';
...
function regionPanel(el){
  ...
  DATA.ages.forEach(age=>{
    ...
    arr.sort((a,b)=>b.count-a.count);
    const tot=arr.reduce((s,a)=>s+a.count,0),mx=Math.max(TARGET,1,...arr.map(a=>a.count)),doneN=arr.filter(a=>a.count>=TARGET).length;
