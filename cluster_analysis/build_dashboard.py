...
<div class="wrap">
 ...
 <div class="grid">
  <div class="card"><h3>الفرق حسب الفئة العمرية</h3><div id="byage"></div></div>
  <div class="card"><h3 id="regh">الفرق حسب المنطقة</h3><div id="byregion"></div></div>
 </div>
 <div class="card"><h3 id="grph">الفرق حسب المجموعة</h3><div id="bygroup"></div></div>
 <div class="card"><h3 id="cityh">الفرق حسب المدينة</h3>
  ...
  <div class="tblwrap"><table id="citytbl"><thead><tr>
   <th data-k="city">المدينة</th><th data-k="region">المنطقة</th><th data-k="group">المجموعة</th><th data-k="count">الفرق</th>
...
function barChart(el,obj,{sort=true,limit=0,color}={}){
...
function render(){
  ...
  // by group
  document.getElementById('grph').textContent='الفرق حسب المجموعة'+(curAge==='الكل'?'':' — '+curAge);
  barChart(document.getElementById('bygroup'),agg(rows,'group'),{});
...
function renderTable(){
  ...
  let rows=rowsFor(curAge).filter(r=>r.count>0);
  if(curAge==='الكل'){ // aggregate per city across ages
    const m={};rows.forEach(r=>{const k=r.city;if(!m[k])m[k]={city:r.city,region:r.region,group:r.group,count:0};m[k].count+=r.count;});
  ...
  document.querySelector('#citytbl tbody').innerHTML=rows.map(r=>
    '<tr><td>'+r.city+'</td><td class="muted">'+r.region+'</td><td>'+r.group+'</td><td class="num">'+r.count+'</td></tr>').join('')
    ||'<tr><td colspan="4" class="muted">لا نتائج.</td></tr>';
}
...
document.getElementById('csearch').oninput=renderTable;
function buildAges(){const c=document.getElementById('ages');c.innerHTML='';
  ['الكل',...AGES].forEach(a=>{const b=document.createElement('button');b.className='agebtn'+(a===curAge?' on':'');
    b.textContent=a;b.onclick=()=>{curAge=a;buildAges();render();};c.appendChild(b);});}
// upload refresh
