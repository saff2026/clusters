...
function groupsPanel(){
  const TARGET=6;
  const all=A().groups;
  // المكتملة = لها ملاعب مطلوبة في التسجيل (تعريف الاتحاد)، أو بلغت الحد الأدنى
  const complete=i=>(all[i].pitches>0)||(all[i].size>=TARGET);
  const regions=[...new Set(all.filter((g,i)=>complete(i)).map(g=>g.region).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ar'));
  let h='<div class="hint">💡 اختر المنطقة ثم المجموعة لعرض فرقها وجدولها. الفرق المرقّمة مؤقتة (ترتيب التسجيل). تُعرض المجموعات المكتملة فقط.</div>';
