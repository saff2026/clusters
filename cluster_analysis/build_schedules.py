...
function groupsPanel(){
  const TARGET=6;
  const all=A().groups;
  const complete=i=>all[i].size>=TARGET;   // المكتملة فقط
  const regions=[...new Set(all.filter(g=>g.size>=TARGET).map(g=>g.region).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ar'));
