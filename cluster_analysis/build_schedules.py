...
function teamMatchesHTML(t,teams,idx){
  ...
  rows.forEach(r=>{h+='<tr><td>'+esc(r.day)+'</td><td class="time">'+esc(r.time)+'</td><td>'+esc(r.pitch)+'</td><td class="mtch"><b>'+esc(r.opp)+'</b></td></tr>';});
  h+='</table></div>';
  ...
  if(g){
    ...
    if(g.tmpl){
      ...
      if(curTeam!==null){
        ...
        h+='<h3 class="sec">مباريات '+esc(g.teams[curTeam])+' — '+nMatch(r.count)+'</h3>'+r.html;
      }else{
