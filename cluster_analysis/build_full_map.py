...
function clusterDoneAges(cities){const ags=compAges();
  return ags.filter(a=>clusterAgeTotal(cities,a)>=TARGET).length;}
...
// تمييز العدد للفئات
function nCat(n){n=+n||0;if(n===1)return 'فئة واحدة';if(n===2)return 'فئتان';
  const t=n%100;return (t>=3&&t<=10)?(n+' فئات'):(n+' فئة');}
// شارة الاكتمال لمجموعة (مكتملة = بلغت الهدف في فئة واحدة على الأقل)
function compChip(cities){const ags=compAges();if(!ags.length)return '';
  const dn=clusterDoneAges(cities),done=dn>=1;
  const lbl=dn===0?'لم تكتمل في أي فئة':(dn===ags.length?'مكتملة في كل الفئات':('مكتملة في '+nCat(dn)));
  return '<span class="compb" style="background:'+(done?'#1a9850':'#c0392b')+'">'+
    (done?'✓ مكتملة':'✗ غير مكتملة')+'</span> <span class="ctm">'+lbl+'</span>';}
// قائمة المجموعات
